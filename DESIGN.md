...
SakunaGraPH should feel like a trusted research instrument: editorial enough to make complex disaster knowledge approachable, and technical enough to preserve the precision of RDF, SPARQL, evidence, and provenance. White paper-like space and dark slate ink form the base; graph blue signs the product, while execution red is reserved for deliberate query actions.
...
- Graph blue for identity and focus; execution red for consequential query actions.
...
The palette is a cool civic neutral field with two deliberate signals: graph blue for identity and red for execution.

### Primary

- **Graph Blue:** Product signature, linked graph concepts, and focus indication.
- **Execute Red:** Primary query execution and destructive urgency only.
- **Deep Execute Red:** Hover state for execution controls.
...
**The One Blue Signature Rule.** Graph blue identifies SakunaGraPH and keyboard focus; it does not wash whole panels or compete with data.
...
- **Primary:** Execute red, white label, and 20px horizontal padding.
...
Navigation is centered, transparent, and compact, with a dark active label and a single dot underneath. Each link retains a 44px target on every viewport and exposes the navigation landmark by name.

### SPARQL Workbench

The workbench combines a quiet title bar, preset selector, syntax editor, and a single red execution action. CodeMirror owns syntax color but inherits the shared focus ring and query typography.
