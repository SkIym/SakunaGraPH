...
OUTPUT_FOLDER = "DROMIC_PARSED_V4.2_2022"
...
def _extract_label_from_lines(lines: list) -> str:
    ...
    for line in lines:
        ...
        if len(line) > 5 and not re.match(r'^Page \d', line):
...
def process_pdf(event: DromicEvent, file_counter: int, pdf_path: str):
    ...
    headers_to_label   = {}   # tuple(col_headers) → label  (header-key match)
    colcount_to_label  = {}   # int(num_cols) → label  (column-count fallback)
    last_active_label  = None # most recently processed named table
        ...
        for page_index, page in enumerate(pages, start=1):
            ...
            for table_obj in tables_found:
                ...
                if label == "unknown_table":
                    # ── Strategy 1: match by header key ───────────────
                    ...
                    # ── Strategy 2: match by column count ─────────────
                    # Handles the case where the continuation page has NO
                    # headers at all (data-only fragment).  We compare the
                    # number of columns in this table against every previously
                    # named table and pick the most recent match.
                    else:
                ...
                if label not in table_headers_used:
                    ...
                    table_headers_used[label] = (col_headers, data_start, loc_col_idx)
                    # Register for future header-key lookups
                    hkey = tuple(col_headers or [])
                    if hkey and label != "unknown_table":
                        headers_to_label[hkey] = label
                    # Register for column-count fallback (most recent label wins)
                    if label != "unknown_table":
                    ...
                    # Detect this: all rows have the same column count as the
                    # registered headers and none look like header rows.
                    if data_start > 0 and extracted_rows:
                        # Check whether first row looks like data (not a header)
                        first_row_text = " ".join(
