...
``--replace`` groups files by context, bundles each group, and replaces its
named graph with one Graph Store Protocol PUT. This avoids a long-lived RDF4J
REST transaction while still letting GraphDB optimize replacement without a
separate, inference-heavy clear.
"""
...
def discover_scope(scope: str, rdf_root: Path, ontology_dir: Path) -> list[Path]:
    """Return Turtle files selected by one logical SakunaGraPH subtree."""
    if scope == "ontology":
        return sorted(path for path in ontology_dir.glob("*.ttl") if path.is_file())

    scope_path = Path(scope)
    if (
        not scope_path.parts
        or scope_path.is_absolute()
        or any(part in (".", "..") for part in scope_path.parts)
    ):
        raise LoaderError(f"--scope must be a relative RDF subtree: {scope}")
    if scope_path.parts[0] not in SCOPES or scope_path.parts[0] == "ontology":
        choices = ", ".join(SCOPES)
        raise LoaderError(
            f"Unknown scope '{scope}'; expected a subtree of: {choices}"
        )

    directory = (rdf_root / scope_path).resolve()
    if not is_relative_to(directory, rdf_root):
        raise LoaderError(f"--scope must stay below {rdf_root}: {scope}")
...
def graph_store_url(host: str, repo: str) -> str:
    return f"{repository_url(host, repo)}/rdf-graphs/service"
...
def group_targets_by_context(
    ...
    """Group targets deterministically for one replacement request per graph."""
...
def replace_context(
    ...
    """Replace one named graph through a single Graph Store Protocol PUT."""
    ...
    try:
        # Concatenated Turtle documents remain valid Turtle: prefix directives
        # may be repeated between statement groups. A temporary file keeps the
        # upload bounded in memory and lets requests send a Content-Length.
        ...
        raise LoaderError(
            f"Could not prepare input for context <{context}>: {error}"
        ) from error
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        metavar="SUBTREE",
        help=(
            "Load an RDF subtree such as events or events/ndrrmc; repeat to "
            "combine scopes (also accepts ontology and all; default: all)"
        ),
    )
    parser.add_argument(
        "--file",
    ...
    parser.add_argument(
        ...
        help=(
            "Per-request clear, replacement, and upload timeout in seconds "
...
def main() -> int:
    ...
    graph_store_endpoint = graph_store_url(args.host, args.repo)
    ...
    try:
        ...
        if args.replace:
            ...
            for context, context_targets in context_groups:
                print(
                    f"  WAIT  {len(context_targets)} file(s) -> <{context}>",
                    flush=True,
                )
                try:
                    replace_context(
