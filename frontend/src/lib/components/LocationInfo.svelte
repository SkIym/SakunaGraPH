<script>
	...
	import { getLocationToShow, getShowInfo, setShowInfo } from "$lib/map/locationInfoStates.svelte";
	import { Heading, Drawer, Card, List, Li, Span } from "flowbite-svelte";

    let open = $derived(getShowInfo());
	let locationName = $derived(getLocationToShow());
	let filteredCsvData = $derived(getFilteredCsvData().filter((row) => (row.city_municipality + ', ' + row.province + ', ' + row.region).includes(locationName)));
</script>

<Drawer bind:open={open} placement="right" onclose={() => setShowInfo(false)} class="text-white">
    <Heading tag="h2" class="p-3 text-4xl font-extrabold ">{locationName}</Heading>
