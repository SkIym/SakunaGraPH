...
<div class="relative h-full">
	{#if items.length}
		<div class="flex flex-col gap-4 sm:flex-row sm:items-center">
			<svg
				viewBox="0 0 240 240"
				class="mx-auto h-48 w-48 shrink-0"
				aria-label="Event counts by disaster category"
				role="img"
			>
				{#each slices as slice (slice.id)}
					<path
						...
						opacity={hovered && hovered.id !== slice.id ? 0.35 : 1}
						role="img"
						aria-label={`${slice.label}: ${slice.count.toLocaleString()} events`}
				...
				{#each slices as slice (slice.id)}
					<div class="flex items-center justify-between gap-3 text-[11px]">
						<span class="flex min-w-0 items-center gap-1.5"
							><i class="h-2 w-2 shrink-0 rounded-full" style="background:{slice.color}"></i><span
								class="truncate">{slice.label}</span
							></span
						>
						<span class="font-semibold tabular-nums text-slate-600"
							>{slice.count.toLocaleString()}</span
						>
					</div>
				{/each}
			</div>
		...
		<ChartTooltip
			...
			y={115}
			title={hovered?.label ?? ''}
			lines={[`${hovered?.count?.toLocaleString() ?? 0} events`]}
		/>
	{:else}
		<p class="flex h-48 items-center justify-center text-xs text-slate-400">
			No disaster-type records in this scope.
		</p>
	{/if}
</div>
