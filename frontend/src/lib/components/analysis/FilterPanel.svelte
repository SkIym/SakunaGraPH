...
<div class="flex h-full min-h-0 flex-col bg-white">
	<div class="flex h-14 shrink-0 items-center justify-between border-b border-slate-200 px-4">
		<div class="flex min-w-0 items-center gap-2">
			...
			{#if analysisFilters.activeCount > 0}
				<span class="flex h-5 min-w-5 items-center justify-center rounded-full bg-indigo-50 px-1.5 text-[10px] font-semibold tabular-nums text-indigo-700">
	...
	<div class="min-h-0 flex-1 overflow-y-auto overscroll-contain">
		<section aria-labelledby="analysis-event-type-heading" class="border-b border-slate-200 px-4 py-4">
			<h2 id="analysis-event-type-heading" class="text-xs font-semibold text-slate-700">Event type</h2>
			<div class="mt-3 grid h-9 grid-cols-3 overflow-hidden rounded-md border border-slate-200" role="group" aria-label="Event type">
		...
		<section aria-labelledby="analysis-search-heading" class="border-b border-slate-200 px-4 py-4">
			<h2 id="analysis-search-heading" class="text-xs font-semibold text-slate-700">Event search</h2>
		...
		{#if error}
			<div class="border-b border-red-100 bg-red-50 px-4 py-3" role="alert">
				...
				<button type="button" onclick={onRetry} class="mt-1.5 text-[11px] font-semibold text-red-700 underline underline-offset-2">
					Try again
				</button>
			</div>
		{/if}
	...
	<div class="flex h-12 shrink-0 items-center justify-between border-t border-slate-200 bg-white px-4">
