		...
		{#if loading}
			{#each [1, 2, 3, 4] as row}
				<div class="flex h-8 items-center gap-2 px-1" aria-hidden="true">
					...
					<span class="h-3 animate-pulse rounded bg-slate-100" style="width:{48 + row * 8}%"></span>
				</div>
			{/each}
				...
				<div>
					<div class="group flex min-h-8 items-center gap-1 rounded px-0.5 hover:bg-slate-50">
						{#if hasChildren}
							...
							>
								<span class="transition-transform {regionOpen ? 'rotate-90' : ''}">&rsaquo;</span>
							</button>
						{:else}
					...
					{#if regionOpen}
						<div class="ml-4 border-l border-slate-100 pl-2">
									...
									<div class="flex min-h-8 items-center gap-1 rounded px-0.5 hover:bg-slate-50">
										{#if localities.length > 0}
											<button
												...
												class="flex h-6 w-6 shrink-0 items-center justify-center text-sm text-slate-400 transition hover:text-slate-700"
											>
												<span class="transition-transform {provinceOpen ? 'rotate-90' : ''}">&rsaquo;</span>
											</button>
										{:else}
											<span class="h-6 w-6 shrink-0"></span>
										{/if}
										<input
											...
											class="h-3.5 w-3.5 shrink-0 cursor-pointer accent-indigo-600"
										/>
										<label for="location-{province.id}" title={province.label} class="min-w-0 flex-1 cursor-pointer truncate text-xs text-slate-600">
											{province.label}
										</label>
									</div>
									...
									{#if provinceOpen}
										<div class="ml-7 border-l border-slate-100 pl-2">
											{#each localities as locality (locality.id)}
												<label for="location-{locality.id}" class="flex min-h-8 cursor-pointer items-center gap-2 rounded px-1.5 hover:bg-slate-50">
													<input
														...
														class="h-3.5 w-3.5 shrink-0 cursor-pointer accent-indigo-600"
													/>
													<span title={locality.label} class="min-w-0 flex-1 truncate text-[11px] text-slate-500">{locality.label}</span>
												</label>
											{/each}
										</div>
									{/if}
							...
							{#each directLocalities as locality (locality.id)}
								<label for="location-{locality.id}" class="ml-7 flex min-h-8 cursor-pointer items-center gap-2 rounded px-1.5 hover:bg-slate-50">
									<input
										id="location-{locality.id}"
										type="checkbox"
										checked={analysisFilters.locationIds.includes(locality.id)}
