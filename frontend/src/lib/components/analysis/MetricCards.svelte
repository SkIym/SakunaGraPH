...
<div
	class="grid gap-px overflow-hidden rounded-xl border border-slate-200 bg-slate-200 sm:grid-cols-2 xl:grid-cols-3"
>
	{#each cards as card}
		<div class="bg-white px-4 py-4">
			<p class="text-[10px] font-semibold uppercase text-slate-400" style="letter-spacing:0.08em;">
				{card.label}
			</p>
			<p class="mt-2 text-2xl font-semibold tabular-nums text-slate-800">
				...
				<div class="flex flex-wrap gap-x-5 gap-y-1">
					{#each summary.damage as damage (damage.unit)}
						<p class="text-sm font-semibold tabular-nums text-slate-700">
							...
							<span class="text-[10px] font-medium text-slate-400">{damage.unit}</span>
						</p>
					{/each}
				</div>
			{:else}
