<script>
	...
	const x = (index) =>
		...
		(items.length < 2
			...
			: (index * (WIDTH - PAD.left - PAD.right)) / (items.length - 1));
	const y = (value) => HEIGHT - PAD.bottom - (value / maximum) * (HEIGHT - PAD.top - PAD.bottom);
	function linePath(key) {
		return items
			.map((item, index) => `${index ? 'L' : 'M'} ${x(index)} ${y(item[key] ?? 0)}`)
			.join(' ');
	}
</script>
		...
		<div class="mb-3 flex flex-wrap gap-x-4 gap-y-1">
			{#each series as line}
				<span class="flex items-center gap-1.5 text-[10px] text-slate-500"
					><i class="h-2 w-2 rounded-full" style="background:{line.color}"></i>{line.label}</span
				>
			{/each}
		</div>
		<svg
			...
			class="h-56 w-full"
			role="img"
			aria-label="Annual casualty trend"
		>
			{#each [0, 0.5, 1] as tick}
				...
				<text
					...
					y={y(maximum * tick) + 4}
					text-anchor="end"
					class="fill-slate-400 text-[9px]">{Math.round(maximum * tick).toLocaleString()}</text
				>
			{/each}
			{#each series as line}
				<path
				...
				{#each items as item, index (`${line.key}-${item.year}`)}
					<circle
						cx={x(index)}
						cy={y(item[line.key] ?? 0)}
						r="3"
						fill="white"
						stroke={line.color}
						stroke-width="2"
						><title>{item.year}: {line.label} {item[line.key].toLocaleString()}</title></circle
					>
				{/each}
			{/each}
			{#each items as item, index (item.year)}
				<text x={x(index)} y={HEIGHT - 12} text-anchor="middle" class="fill-slate-400 text-[9px]"
					>{item.year}</text
				>
			{/each}
		</svg>
...
{:else}
	<p class="flex h-56 items-center justify-center text-xs text-slate-400">
		No casualty trend data in this scope.
	</p>
{/if}
