<script>
	...
	function shade(count) {
		const alpha = 0.1 + (count / maximum) * 0.52;
	...
	<div class="grid grid-cols-4 gap-1.5 sm:grid-cols-6 xl:grid-cols-8">
		{#each items as item (item.period)}
			<button
				...
				class="rounded-md border px-2 py-1.5 text-left transition focus:outline-none focus:ring-2 focus:ring-indigo-400 {selectedYear ===
				Number(item.period)
					? 'border-slate-800 ring-1 ring-slate-800'
					: 'border-slate-200 hover:border-indigo-300'}"
				style="background:{shade(item.count)}; color:#172033"
			>
