...
<script>
   ...
   * @property {boolean} [rotate=false] - Rotate tick labels to avoid overlap.
   * @property {number} [rotateDegrees=-60] - Degrees to rotate tick labels (negative = counter-clockwise).
   * @property {number} [maxTicks] - Hard cap on how many labels to show. Auto-calculated if omitted.
   */
  /** @type {Props} */
  let {
    tickMarks = false,
  ...
  // Dynamically calculate how many ticks can fit without overlapping.
  // Assumes ~40px per label when horizontal, ~14px when rotated.
  let tickVals = $derived.by(() => {
    const total = allTickVals.length;
    const chartWidth = $width ?? 600;

    // Figure out the cap: use maxTicks prop, or estimate from available width
    const labelWidth = rotate ? 14 : 52; // px per label (rotated = just height of text)
    const estimatedMax = maxTicks ?? Math.max(1, Math.floor(chartWidth / labelWidth));

    if (total <= estimatedMax) return allTickVals;

    // Pick every Nth tick so we never exceed estimatedMax labels
    const step = Math.ceil(total / estimatedMax);
    return allTickVals.filter((_, i) => i % step === 0);
  });

  let halfBand = $derived(isBandwidth ? $xScale.bandwidth() / 2 : 0);

  // Build the transform string for rotated labels
  function labelTransform(x, gutterAndLen) {
    if (!rotate) return '';
    return `rotate(${rotateDegrees}, ${x + halfBand}, ${gutterAndLen})`;
  }
</script>
  ...
  {#each tickVals as tick, i (tick)}
    ...
    <g class="tick tick-{i}" transform="translate({$xScale(tick)},{Math.max(...$yRange)})">
      ...
      <text
        ...
        transform={labelTransform(0, tickGutter + tickLen)}
      >
        {format(tick)}
      </text>
    </g>
