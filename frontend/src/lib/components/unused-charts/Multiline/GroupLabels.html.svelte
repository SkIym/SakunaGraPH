...
<script>
  ...
  const { data, x, y, xScale, yScale, xRange, yRange, z , yDomain} = getContext('LayerCake');
  ...
  values.reduce((a, b) => ($x(b) > $x(a) ? b : a));

  let left = $derived(values =>
    $xScale($x(lastPoint(values))) / Math.max(...$xRange)
  );

  let top = $derived(values =>
    $yScale($y(lastPoint(values))) / Math.max(...$yRange)
  );
</script>
