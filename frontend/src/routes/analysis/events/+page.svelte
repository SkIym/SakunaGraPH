<script>
	...
	$effect(() => {
		...
		const timer = window.setTimeout(async () => {
			try {
				response = await apiJson(withQuery('/api/analysis/events', query), {
...
<section class="mx-auto w-full max-w-[1500px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
	<div class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
		...
		<a href="/analysis" class="text-xs font-semibold text-indigo-600 transition hover:text-indigo-800">Analysis overview</a>
	</div>
	...
	<div class="overflow-visible rounded-lg border border-slate-200 bg-white shadow-sm">
		...
		{#if !error && !loading && totalPages > 1}
			<div class="flex flex-col gap-3 border-t border-slate-200 px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-5">
				<p class="text-[10px] tabular-nums text-slate-400">Page {page.toLocaleString()} of {totalPages.toLocaleString()}</p>
