<script lang="ts">
  ...
  import { Card, Button, Dropdown, DropdownItem, tooltip } from "flowbite-svelte";
  import { ChevronDownOutline } from "flowbite-svelte-icons";

  // Data imports
  import { getSortedRegionList, seriesNames } from '$lib/components/chart-shapes/chart.svelte.js';
  ...
  let sortedData = $derived(getSortedRegionList(selectedType));
  ...
  let options = $derived({
    ...
    tooltip: {
      enabled: true,
      // Custom HTML for consistent white background/dark text
        ...
        return `
          <div style="background: #ffffff; border: 1px solid #e5e7eb; border-radius: 8px; padding: 12px; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1); font-family: 'Inter', sans-serif;">
            <div style="display: flex; align-items: center; gap: 8px;">
              ...
              <span style="font-size: 13px; font-weight: 700; color: #111827;">${region}</span>
            </div>
            <div style="margin-top: 4px; font-size: 12px; color: #4b5563;">
              Disasters: <span style="font-weight: 600; color: #111827;">${val}</span>
            </div>
          </div>
        `;
      }
...
<Card class="p-4 md:p-6 [background-image:var(--card-bg)] dark:[background-image:var(--card-bg-dark)] max-w-full!">
  <div class="flex flex-row items-center justify-between border-b border-gray-200 pb-3 dark:border-gray-700">
    <h5 class="me-1 text-xl leading-none font-bold text-gray-900 dark:text-gray-300">Region Rankings by Disaster</h5>
