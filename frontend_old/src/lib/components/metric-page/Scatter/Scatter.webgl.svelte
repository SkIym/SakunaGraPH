...
<script>
  ...
  import { disasterColors } from '$lib/components/chart-shapes/chart.svelte.js';
   ...
   * @property {Object} [colors=null] - Color map for disaster types
   * @property {string} [zKey=null] - Key to access the type property in data
   */

  /** @type {Props} */
  let { r = 5, fill = '#0cf', stroke = '#000', colors = null, zKey = null } = $props();
    ...
    if ($gl) {
      ...
      const draw = regl({
        ...
        void main () {
          ...
          gl_FragColor = vec4( mix(stroke_color, v_color, stroke), 1.0 ) * alpha;
        ...
        attributes: {
          /**
           * @param {any} context
           * @param {{ points: Array<any>, pointWidth?: number, zKey: string, colors: Record<string, string> }} props
           */
          ...
          pointColor: (context, props) => {
            return props.points.map(point => {
              const type = props.zKey ? point[props.zKey] : null;
              const hexColor = type && props.colors ? props.colors[type] : fill;
              return hexToRgbPercent(hexColor);
            });
          }
        },
        uniforms: {
