...
from typing import Dict, Generic, Iterable, TypeVar, Set
from collections import defaultdict

from sakunagraph_etl.rdf.iris import mint_canonical_iri
...
from sakunagraph_etl.rdf.graph import SKG, PROV
...
def load_source_paths(paths: Iterable[Path]) -> list[DisasterEvent]:
    """Load an explicit, deterministic set of source Turtle files."""
    all_events: list[DisasterEvent] = []
    for ttl_path in sorted(Path(path) for path in paths):
        source_name = (
            ttl_path.parent.name.lower()
            if ttl_path.parent.name.lower() in SOURCE_PRIORITY
            else re.sub(r"-\d+$", "", ttl_path.stem).lower()
        )
        g = Graph()
...
def load_all_sources(sources_dir: Path) -> list[DisasterEvent]:
    """Compatibility discovery below a source event RDF root."""
    return load_source_paths(sources_dir.rglob("*.ttl"))
...
def expand_clusters(
    ...
    """Expand registry clusters deterministically and retain untouched state.

    Added members keep an existing canonical identifier. When two existing
    clusters merge, the lexicographically smallest canonical identifier wins,
    making the result independent of scored-pair order.
    """
    uf = UnionFind[str]()
    ...
    matching_pairs = [pair for pair in scored_pairs if pair[2].is_match]
    for a, b, _ in matching_pairs:
        uf.union(str(a.uri), str(b.uri))

    members_by_root: dict[str, set[str]] = defaultdict(set)
    for member in tuple(uf.parent):
        members_by_root[uf.find(member)].add(member)

    canonical_by_root: dict[str, list[str]] = defaultdict(list)
    for canonical, members in registry.items():
        if members:
            canonical_by_root[uf.find(min(members))].append(canonical)

    updated_clusters: list[tuple[URIRef, frozenset[URIRef]]] = []
    for root, canonicals in canonical_by_root.items():
        updated_clusters.append(
            (
                URIRef(min(canonicals)),
                frozenset(URIRef(member) for member in members_by_root[root]),
            )
        )
    updated_clusters.sort(key=lambda cluster: str(cluster[0]))

    known_roots = set(canonical_by_root)
    remaining = [
        pair
        for pair in matching_pairs
        if uf.find(str(pair[0].uri)) not in known_roots
    ]
    return updated_clusters, remaining
...
def build_clusters(
    ...
    return sorted(result, key=lambda cluster: str(cluster[0]))
...
def save_registry(
    ...
    registry = {
        str(canonical_iri): sorted(str(member) for member in members)
        for canonical_iri, members in sorted(clusters, key=lambda cluster: str(cluster[0]))
    }
