...
from sakunagraph_etl.rdf.iris import GDA_NS
...
def export_slices(df: pd.DataFrame, specs: dict[str, Any], out_dir: str | Path) -> None:
...
def transform_gda(
    ...
    *,
    ...
    raw_path = Path(path)
    debug_output_dir = Path(debug_dir) / "gda" if debug_dir is not None else None
    ...
    if debug_output_dir is not None:
        ...
        pd.DataFrame(incidents).to_csv(
            debug_output_dir / "incidents.csv",
            index=False,
        )
        export_slices(df, EXPORT_SPECS, debug_output_dir / "slices")
    ...
    if debug_output_dir is not None:
        df.to_csv(debug_output_dir / "normalized.csv", index=False)
...
def main(argv: list[str] | None = None) -> int:
    ...
    transform_gda(
        args.input or settings.paths.raw_root / "static" / "geog-archive-cleaned.xlsx",
