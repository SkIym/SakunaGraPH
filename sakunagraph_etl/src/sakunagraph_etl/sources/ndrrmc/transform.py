...
import polars as pl
from sakunagraph_etl.rdf.iris import NDRRMC_EVENT_NS
...
from sakunagraph_etl.transform.helpers import (
...
def _write_debug_csv(
    ...
    if debug_dir is None:
        return
    debug_path = Path(debug_dir) / "ndrrmc" / Path(event_folder_path).name / filename
...
def _load_climate_parameters(remarks_text: str) -> list[ClimateParameterMeasurement]:
    from sakunagraph_etl.enrichment.climate_parameters import PARAMS_EXTRACTOR
...
def load_incidents(
    ...
    *,
    ...
    _write_debug_csv(df, debug_dir, event_folder_path, "incidents.csv")
...
def load_housing(
    ...
    *,
    ...
    _write_debug_csv(df, debug_dir, event_folder_path, "housing.csv")
...
def load_class_suspension(
    event_folder_path: str,
    *,
    debug_dir: str | Path | None = None,
) -> List[ClassDisruption] | None:
    ...
    _write_debug_csv(df, debug_dir, event_folder_path, "class-suspension.csv")
