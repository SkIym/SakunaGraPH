...
from sakunagraph_etl.rdf.graph import SKG, GEO
...
def main() -> None:
    ...
    xlsx_path = Path(
        args.input
        or settings.paths.raw_root / "PSGC-4Q-2025-Publication-Datafile.xlsx"
    )
    out_path = Path(args.output)
    if not out_path.is_absolute():
        out_path = (args.out_dir or settings.paths.rdf_root / "psgc") / out_path
